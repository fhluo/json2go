import { GetAllCapsWords, GetFontSize, GetLocale, GetOptionsGenerateInRealTime, GetOptionsValidJSONBeforeGeneration, GetVersion, SetAllCapsWords, SetFontSize, SetLocale, SetOptionsGenerateInRealTime, SetOptionsValidJSONBeforeGeneration } from "../wailsjs/go/main/App"
...
export function createConfigState() {
    ...
    return {
        ...
        addAllCapsWords(words: string) {
            this.allCapsWords = Array.from(new Set(allCapsWords.concat(splitWords(words))))
        },
        removeAllCapsWords(words: string) {
            const wordList = splitWords(words)
            this.allCapsWords = Array.from(new Set(allCapsWords.filter(word => !wordList.includes(word))))
        },
        get validJSON() {
            return validJSON
        },
        set validJSON(value) {
            validJSON = value
            SetOptionsValidJSONBeforeGeneration(validJSON)
        },
        get realTime() {
            return realTime
        },
        set realTime(value) {
            realTime = value
            SetOptionsGenerateInRealTime(realTime)
        },
        get version() {
            return version
        }
